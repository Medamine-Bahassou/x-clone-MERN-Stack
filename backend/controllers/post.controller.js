...
export const createPost = async (req, res) => {
    try{
        ...
        if(!text && !img) return res.status(404).json({error: "Post must contain text or image"});
...
export const getFollowingPosts = async (req, res) => {
    try{
        ...
        res.status(200).json(feedPosts);  // we can also use pagination here to limit the number of posts returned.
